	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"content": schema.BoolAttribute{
				Optional:    true,
				Computed:    true,
				Description: "Whether Open WebUI includes each file's extracted text, which arrives inside `data_json`. Defaults to `true`. Set it to `false` to keep the text of large documents out of Terraform state.",
			},
			...
			"files": schema.ListNestedAttribute{
				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						...
						"user_id":    schema.StringAttribute{Computed: true, Description: "UUID of the Open WebUI account that owns this object."},
						"data_json":  schema.StringAttribute{Computed: true, Description: "JSON data blob associated with the file. It carries the file's extracted text while `content` is `true`."},
