	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"verified": schema.BoolAttribute{
				Computed:    true,
				Description: "Always `true`. The read fails with an error when Open WebUI cannot reach the endpoint or the endpoint refuses the credentials, so this attribute never reads `false`.",
			},
		},
	}
}
